...
const Introduction = ({ handleButtonClick }) => {
  ...
  const GradientText = ({ children, animation }) => (
    <div
      className="text-[164px] font-bold text-transparent bg-clip-text"
      style={{
  ...
  return (
    <section className="min-h-screen bg-[#000B2A] text-white flex flex-col items-center justify-between px-8 md:px-16 space-y-8 md:space-y-0 introduction">
      
      {/* Code Snippets Section */}
      <div className="flex items-center space-x-4  symbol" >
        <GradientText animation="fade-right">{"<"}</GradientText>
        <GradientText animation="fade-up">{"/"}</GradientText>
        <GradientText animation="fade-left">{">"}</GradientText>
      </div>
